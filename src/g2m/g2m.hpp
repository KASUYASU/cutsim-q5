...
class g2m : public QObject {
    ...
    public:
    	g2m()  { debug = false;
        	...
        	userOrigin = Pose( Point(0,0,0), Point(0,0,0) );
        	total_gcode_lines = 0;
        	filelists.resize(0);
    	}
        ...
        int toFileLineNo(int gcodeLineNo, QString &file) {
        	for (int i=0; i<(int)filelists.size(); i++) {
        		if (gcodeLineNo > (filelists[i].position + filelists[i].lines))
        			continue;
        		else {
        			file = filelists[i].file;
        			return gcodeLineNo - filelists[i].position;
        		}
        	}
        	file = "";
        	return -1;
        }
        void clearGcode(void) {
            total_gcode_lines = 0;
	    lineTable.resize(0);
            filelists.resize(0);
        }
